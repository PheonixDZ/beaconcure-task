...
inputs = {
  ...
  eks_node_group_instance_types = ["t3.small"] # Smaller instance for staging
  eks_node_group_ami_type       = "AL2_x86_64"
  eks_node_group_disk_size      = 20 # Smaller disk for staging
  eks_node_group_desired_size   = 1  # Fewer nodes for staging
  ...
  eks_ingress_cidr_blocks     = ["106.219.85.123/32"]
