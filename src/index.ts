...
import { metricsMiddleware } from './utilities/metrics-middleware'
import { register, imageUploadsTotal } from './utilities/metrics'
...
app.post(`${baseUrl}/images`, async (req: express.Request, res: express.Response) => {
  ...
  upload(req as any, res, async (err) => {
    ...
    if ((req as any).fileValidationError) {
      imageUploadsTotal.labels('error').inc();
    ...
    imageUploadsTotal.labels('success').inc();
